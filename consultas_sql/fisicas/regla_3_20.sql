...
Mejoré la condición de existencia de 'Residencial' usando not exists. */
...
with base as (
    ...
    left join cca_unidadconstruccion cu on cu.construccion = ca.t_id  
...
tb_final as (
    select 
        ...
        case 
            when (select area_construida_total_uso from tb1 tt where tt.numero_predial = t.numero_predial and tt.uso = 'Residencial') < t.area_construida_total_uso and uso <> 'Residencial' then 'Fail'
            when not exists (select 1 from tb1 tt1 where tt1.numero_predial = t.numero_predial and tt1.uso = 'Residencial') then 'Fail'
            else 'Ok' 
        end as cond_3_20_1
    from tb1 t
...
select 
    numero_predial,
    'Habitacional' as dest_eco,
    uso,
    area_construida_total_uso,
    'Fail' as cond_3_20
from tb_final 
where numero_predial in (select numero_predial from tb_final where cond_3_20_1 = 'Fail')
order by numero_predial