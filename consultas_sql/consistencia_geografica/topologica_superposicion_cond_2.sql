---Terreno-Informal (NP posición 22 igual "2") asociado a un derecho de "Ocupacion" no debe superponerse con un Terreno asociado a un Predio.Tipo  “Privado”.
WITH terrenos_informales_ocupacion AS (
    -- Terrenos asociados a predios informales (pos 22 = '2') con derecho de Ocupación
    SELECT DISTINCT
    ...
    WHERE 
        ...
        -- Validar que tenga al menos un derecho de Ocupacion
        AND dt.ilicode = 'Ocupacion'
        -- Asegurar que la geometría sea válida
        AND t.geometria IS NOT NULL
),
terrenos_privados AS (
    -- Terrenos asociados a predios privados
    SELECT DISTINCT
        t.t_id AS terreno_id,
        t.geometria,
        p.numero_predial,
...
SELECT     -- Información del terreno informal
    ...
    -- Información del terreno privado
    tpub.terreno_id AS terreno_privado_id,
    tpub.numero_predial AS numero_predial_privado,
    tpub.predio_id AS predio_privado_id,
    tpub.tipo_predio AS tipo_predio_privado,    
    -- Análisis espacial
...
FROM terrenos_informales_ocupacion tip
INNER JOIN terrenos_privados tpub 
